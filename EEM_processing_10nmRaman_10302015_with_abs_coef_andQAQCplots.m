...
%%% starna and blank abs files are named abs_s(date) and abs_b(date) #check
%%% this
...
%% Create Sarna water %%%%%%%%%
    ...
    for kk = 1:length(S)% loop through data frmaes
        ...
        dlmwrite([S(kk).name,'_processed_EEM','.','csv'],output); % write a normalized data file for each file. output is comma seperated values with .MAT ending
       ...
       title([S(kk).name,'_processed_EEM'])
       
      absorption_coefficients = (((cell2mat(S(kk).absdata(2:end,10))*2.303)/0.01)/dilution_factor); % 
