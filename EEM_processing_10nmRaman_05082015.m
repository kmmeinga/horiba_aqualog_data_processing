...
%%% Current version may be flawed!!! Line 248 needs clarification. SHOULD
%%% WE DIVIDE BY DILUTION FACTOR INSTEAD OF MULTIPLY?
...
%%Read in absorption file for blank
   ...
   %%% flucut the blank
    ...
    %% Set up otions for flucut
        ...
        opts.innerfilter.wavelengths = flipud(rot90(blank(1,2:end)));
        ...
        blankflucutwithife_data = flucut(A, 10,10, opts);
...
%% Create Sarna water %%%%%%%%%
        elseif (list(i,1).name(1,1)) == 's'; % find starna named ramandate.csv
           ...
           % clearvars raw R; %fasterrrrr
            ...
            ask = ['Enter dilution factor for: ',splitname , '    eg. THIS SHOULD BE BETWEEN 0 AND 1'];
    ...
    %%%%%%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%
    normblanktosarna =  blankflucutwithife_data.data./sarna_flucutwithife_data.data;
    
%     for col= 1:size(blankflucutwithife_data.data,3); % loop to find max value for each column or exitation value.
%         [num idx] = max(blankflucutwithife_data.data(:,:,col)); % output value and position of the value
%         maxval(col) = num; % save maxval for this col
%         xblank(col) = col;% associate positions
%         yblank(col) = idx;% ""
%     end
    ...
    for k = 1:length(S) % now loop through all structures that were built
        ...
        S(k).normalized_flucut_dilution = normdata*dilution_factor; % 05/08/2015 KMM - SHOULD WE DIVIDE BY DILUTION FACTOR INSTEAD OF MULTIPLY.
